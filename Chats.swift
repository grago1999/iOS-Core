...
class Chats {
    ...
    static func create(userId:Int, completion: @escaping (Bool, Int, String, Chat?) -> Void) {
        Connection.request(path:"/api/v1/chat/create", post:["userId":String(userId)], completion: { res in
            var chat:Chat?
            if res.success {
    ...
    static func send(chatId:Int, text:String, completion: @escaping (Bool, Int, String) -> Void) {
        Connection.request(path:"/api/v1/chat/send", post:["chatId":String(chatId), "text":text], completion: { res in
